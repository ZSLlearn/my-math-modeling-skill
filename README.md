...
[![GitHub](https://img.shields.io/badge/GitHub-ZSLlearn%2Fmy--math--modeling--skill-181717?logo=github)](https://github.com/ZSLlearn/my-math-modeling-skill)
...
本技能为数学建模竞赛（CUMCM、MCM/ICM等）提供**建模分析 → 代码实现 → 论文撰写**三阶段协作工作流程，配合**指导教师**阶段门禁审查，确保建模、编程、论文撰写三个环节紧密衔接，产出高质量的数学建模成果。
...
- 🎓 **指导教师把关**：赛前思路启发 + 三阶段门禁审查 + 终稿全面审查，不通过不放行
- 📚 **算法资源库**：涵盖优化、预测、评价、图论、统计、综合、机器学习7大类60+算法
- 💻 **代码模板**：Python/MATLAB 双语言支持，符合 SCI/Nature 可视化标准
- 📄 **论文规范**：默认模板 + 自定义模板支持，包含模型检验、灵敏度分析等必写章节，符合竞赛格式要求
- 🛠️ **文档处理集成**：集成 pdf、xlsx、paper_search 三个专业子skill，支持：
  - 📑 **pdf skill**：读取PDF题目、提取文本和表格数据
  - 📊 **xlsx skill**：处理Excel表格数据、结果输出，使用公式而非硬编码
  - 🔎 **paper_search skill**：通过OpenAlex API搜索学术论文，提供参考文献支持
- 📝 **Markdown 撰写**：论文输出为 Markdown 格式，支持 LaTeX 公式，可直接转换 PDF
- 🏆 **竞赛适配**：自动识别国赛/美赛，调整建模策略和论文侧重点（格式约束、创新权重、语言风格等）
- ⚠️ **避坑指南**：整理了20个竞赛常见错误、时间管理建议、提交前终极检查清单
...
| **论文撰写** | 资料收集、论文规划、内容撰写，使用 Markdown 格式撰写论文                             | 完整论文（≥15000字，.md）  |

### 四角色介绍

本技能采用**三角色（建模/编程/论文）+ 一指导**的四角色协作模式：

|        角色        | 定位 | 核心能力                 | 介入时机                        |
| :----------------: | ---- | ------------------------ | ------------------------------- |
|  **建模手**  | 大脑 | 数学建模、算法设计       | 第一阶段：建模分析              |
|  **编程手**  | 双手 | 代码实现、数据可视化     | 第二阶段：代码实现              |
|  **论文手**  | 喉舌 | 学术写作、成果整理       | 第三阶段：论文撰写              |
| **指导教师** | 舵手 | 方向把关、质量审查、答疑 | 赛前 + 三阶段门禁 + 终审 + 全程 |
...
指导教师负责在三个阶段的切换点进行质量门禁审查，不通过则退回修正。赛前给出思路启发和避坑提醒，建模完成后审查分析文档，编程完成后检查结果完整性，论文完成后进行终审和反AI味专项检查。全程可答疑，但不替代建模手/编程手/论文手做具体决策。
...
```bash
npx skills add https://github.com/ZSLlearn/my-math-modeling-skill.git
```
...
git clone https://github.com/ZSLlearn/my-math-modeling-skill.git ~/.claude/skills/math-modeling-skill
```

#### 方法三：手动安装

1. 下载本项目的 [ZIP 文件](https://github.com/ZSLlearn/my-math-modeling-skill/archive/refs/heads/main.zip) 或克隆到本地
...
├── references/                  # 各阶段说明文档
│   ├── roles/                 # 角色说明文件夹
│   │   ├── 指导教师说明.md    # 指导教师工作细则（全程答疑+阶段门禁审查）
│   │   ├── 建模手说明.md      # 建模分析阶段工作细则
│   │   ├── 编程手说明.md      # 代码实现阶段工作细则
│   │   └── 论文手说明.md      # 论文撰写阶段工作细则
...
│   │   └── 2023MCM ICM/       # 美赛O奖论文（A-F题）
│   ├── 国赛论文模板.md       # 国赛（CUMCM）专用论文模板（中文，≤20页，摘要≤300字）
│   ├── 美赛论文模板.md       # 美赛（MCM/ICM）专用论文模板（英文，20-30页，Summary为核心）
│   ├── 国赛美赛差异说明.md     # 国赛与美赛核心差异对比、Skill行为适配指南
│   ├── 常见错误与避坑指南.md   # 竞赛常见错误与避坑指南
│   └── README.md              # 角色工作指南
├── tools/                      # 集成的子Skill
│   ├── pdf/                   # PDF文档处理Skill
│   ├── xlsx/                  # Excel表格处理Skill
...
| 🤖 机器学习   | 10       | 随机森林、AdaBoost、孤立森林、CNN、RNN/LSTM、Transformer、GNN、强化学习、集成学习、SHAP/LIME |
...
- ✍️ **论文手**：使用 Markdown 格式撰写论文、使用 `paper_search skill` 交叉验证文献
...
| ✍️**论文手说明.md** | **重点增加去AI味写作指南**、Markdown写作规范、图文并茂规范（每张图≥100字分析）、人称约束、叙述方式规范 |
