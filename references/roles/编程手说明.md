...
编程手在工作前必须确认当前竞赛类型，因为国赛和美赛对代码实现的要求有重要差异：

| 维度 | 国赛（CUMCM） | 美赛（MCM/ICM） |
|------|-------------|---------------|
| **图表风格** | 规范清晰优先——标题、坐标轴标签、单位一个不能少；清晰可读是第一原则 | 专业美观优先——配色优雅、排版精致，追求 SCI 期刊级别的视觉质量 |
| **附录要求** | 源代码**必须完整包含**，遗漏即扣分 | 不强求完整代码，建议附关键算法片段 |
| **结果呈现** | 强调精确性和实用性，数值必须贴合实际 | 强调可解释性——不仅给结果，还要分析"为什么""如何改进" |
| **代码注释** | 必须，方便评委审查附录 | 必须，方便评委理解算法逻辑 |
| **可复现性** | 关键——代码+数据+参数，缺一无法复现 | 建议——核心算法可复现即可 |
| **图表数量** | 每个问题至少 1-2 张核心图表 | 鼓励多图多角度展示，图表之间应有逻辑关联 |
...
| **机器学习** | `assets/07-机器学习算法说明.md` | scikit-learn, torch, tensorflow, keras, shap, lime 等 |
...
1. **确认竞赛类型**：国赛还是美赛？根据竞赛类型调整代码风格和输出要求
2. **读取建模分析文档**: 理解建模手的分析结果
3. **确认编程语言**: Python 或 MATLAB（如果未指定，询问用户）
4. **理解题目要求**: 读取题目文件，明确需要解决的问题
...
### 1. 建议文件结构（推荐严格遵守，但可根据项目需要调整）

编程手建议按照以下文件结构创建和组织项目文件：
