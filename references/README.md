...
- **`tools/docx`** - Word文档处理（辅助工具）
  - 读取、编辑 .docx 文件
  - 论文撰写阶段使用 Markdown 格式，不再依赖 docx skill
...
**指导教师**是团队的舵手，负责在建模开始前给出方向启发、在每个阶段切换时进行质量门禁审查（不通过则退回修正）、论文完成后进行全面终审，以及在全程中回答任何团队成员的技术疑问。指导教师不亲自执行建模/编码/写作任务，仅做方向性把关。
...
模型选择原则已按国赛/美赛条件化处理，详见 `references/roles/建模手说明.md` 中的「模型选择原则」和「按竞赛类型区分」章节。核心区分：

- **国赛**：经典模型优先，分层改进（基础→改进→优化），深度学习慎用
- **美赛**：创新性最高权重，AI/ML 鼓励，需有充分理由和数据量支撑
...
| 机器学习 | scikit-learn, torch, tensorflow, keras, shap, lime | — |
...
> 以下为建议的论文结构，可根据题目特点增删合并章节。模板文件（国赛/美赛）中的结构同样是建议。
...
| 时机 | 触发条件 | 指导教师动作 |
|------|---------|------------|
| **赛前** | 竞赛类型确认后 | 题型定位提示 + 避坑提醒 |
| **门禁1** | 建模分析完成后 | 审查分析文档 → 通过则放行编程 / 不通过则退回修正 |
| **门禁2** | 代码实现完成后 | 检查结果完整性 → 通过则放行论文 / 不通过则退回修正 |
| **终审** | 论文完成后 | 全面审查 + 反AI味检查 → 通过则论文完成 / 不通过则退回修正 |
| **全程** | 任何角色提问 | 方向性答疑（不替代角色决策） |

详见 `references/roles/指导教师说明.md`
